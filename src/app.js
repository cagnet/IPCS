...
let schematic=createExample(),simulation=null,fileHandle=null,dirty=false,dialogMode='new';
...
function updateProjectProperty(path,value){const [section,key]=path.split('.');if(section==='background'){if(!schematic.background)return;schematic.background[key]=value}else schematic.grid[section][key]=value;markDirty();editor.render(simulation);renderBackground()}
...
$('#newProjectForm').onsubmit=async event=>{event.preventDefault();const name=$('#projectName').value.trim();if(!name)return;const selectedPdf=$('#backgroundPdfInput').files[0];try{let background=dialogMode==='edit'?schematic.background:null;if(selectedPdf)background={name:selectedPdf.name,mimeType:selectedPdf.type||'application/pdf',dataUrl:await fileToDataUrl(selectedPdf),visible:true,opacity:50,x:0,y:0,width:1200,height:900};const data={name,brand:$('#projectBrand').value.trim(),year:$('#projectYear').value?numberValue('#projectYear',null):null,background};$('#newProjectDialog').close();if(dialogMode==='new'){loadProject(new Schematic(data));fileHandle=null}else{Object.assign(schematic,data);refresh()}markDirty(true);toast(dialogMode==='new'?`Projet « ${name} » créé`:'Propriétés du projet mises à jour')}catch(error){toast(`Lecture du PDF impossible : ${error.message}`)}};
