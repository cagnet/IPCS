...
function renderBackground(){const layer=$('#pdfViewport'),canvas=$('#pdfBackground'),bg=schematic.background,active=bg?.visible&&bg?.dataUrl;if(active){canvas.style.left=`${bg.x??0}px`;canvas.style.top=`${bg.y??0}px`;canvas.style.width=`${bg.width??1200}px`;canvas.style.height=`${bg.height??900}px`;canvas.style.opacity=String((bg.opacity??50)/100);layer.hidden=false;syncPdfViewport();renderPdfPage(bg.dataUrl)}else layer.hidden=true;$('#canvasWrap').classList.toggle('has-background',!!active)}
...
async function updateProjectProperty(path,value){try{const [section,key]=path.split('.');if(path==='background.file'){schematic.background={name:value.name,mimeType:value.type||'application/pdf',dataUrl:await fileToDataUrl(value),visible:true,opacity:50,x:0,y:0,width:1200,height:900};renderedPdfSource=null}else if(path==='background.remove'){schematic.background=null;renderedPdfSource=null}else if(section==='background'){if(!schematic.background)return;schematic.background[key]=value}else if(key)schematic.grid[section][key]=value;else schematic[section]=value;markDirty();editor.render(simulation);renderBackground();if(path==='background.file'||path==='background.remove')properties.render(null,schematic)}catch(error){toast(`Modification du projet impossible : ${error.message}`)}}
...
$('#newProjectForm').onsubmit=async event=>{event.preventDefault();const name=$('#projectName').value.trim();if(!name)return;const selectedPdf=$('#backgroundPdfInput').files[0];try{let background=null;if(selectedPdf)background={name:selectedPdf.name,mimeType:selectedPdf.type||'application/pdf',dataUrl:await fileToDataUrl(selectedPdf),visible:true,opacity:50,x:0,y:0,width:1200,height:900};const data={name,brand:$('#projectBrand').value.trim(),year:$('#projectYear').value?numberValue('#projectYear',null):null,background};$('#newProjectDialog').close();loadProject(new Schematic(data));fileHandle=null;markDirty(true);toast(`Projet « ${name} » créé`)}catch(error){toast(`Lecture du PDF impossible : ${error.message}`)}};
