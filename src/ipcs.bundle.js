...
class Editor{
 constructor(options){Object.assign(this,options);this.mode='EDIT';this.tool=null;this.styleId='RED';this.selection=null;this.scale=1;this.pan={x:0,y:0};this.history=[];this.future=[];this.space=false;this.bind();this.applyTransform()}
 ...
 down(e){this.svg.focus();if(e.button===2){e.preventDefault();this.cancelAction();return}const p=this.snap(this.point(e)),terminal=e.target.dataset.terminal,id=e.target.closest('[data-id]')?.dataset.id;if(e.button===1||this.space){this.drag={kind:'pan',start:{x:e.clientX,y:e.clientY},origin:{...this.pan}};this.svg.setPointerCapture(e.pointerId);return}if(this.mode==='SIMULATION'){const c=this.schematic.getComponent(id);if(c?.type==='contact'&&c.controlType==='manual'){c.manualActivated=!c.manualActivated;this.onChange('simulation')}return}if(this.tool==='wire'){this.wireClick(terminal,p,e.shiftKey);return}if(this.tool&&!id){this.snapshot();this.add(this.tool,p);return}if(id){this.select(id);const c=this.schematic.getComponent(id);if(c){this.snapshot();this.drag={kind:'component',id,offset:{x:p.x-c.position.x,y:p.y-c.position.y}};this.svg.setPointerCapture(e.pointerId)}}else this.select(null)}
 move(e){const p=this.point(e);this.onStatus(this.snap(p));if(this.drag?.kind==='pan'){this.pan={x:this.drag.origin.x+e.clientX-this.drag.start.x,y:this.drag.origin.y+e.clientY-this.drag.start.y};this.applyTransform()}if(this.drag?.kind==='component'){const c=this.schematic.getComponent(this.drag.id),q=this.snap({x:p.x-this.drag.offset.x,y:p.y-this.drag.offset.y});c.position=q;this.render()}if(this.pendingWire){const q=this.constrain(this.pendingWire.points.at(-1),this.snap(p),e.shiftKey);this.preview=[...this.pendingWire.points,q];this.render()}}
 ...
 cancelAction(){const hadAction=!!(this.pendingWire||this.drag||this.tool);this.pendingWire=null;this.preview=null;this.drag=null;this.tool=null;this.onCancel?.();this.render();this.onStatus(null,hadAction?'Action annulée':'Mode sélection')}
