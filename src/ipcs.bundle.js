...
class ComponentPalette{constructor(root,onPick){this.items=[['select','↖','Sélection'],['powerSource','◯','Alimentation'],['coil','⌁','Bobine'],['contactNO','╱','Contact NO'],['contactNC','—','Contact NC'],['wire','⌁','Fil'],['junction','●','Jonction']];this.root=root;this.onPick=onPick;this.selected='select';this.render()}render(){this.root.replaceChildren(...this.items.map(([id,icon,label])=>{const b=document.createElement('button');b.dataset.tool=id;b.innerHTML=`<span>${icon}</span>${label}`;b.classList.toggle('active',id===this.selected);b.onclick=()=>this.select(id);return b}))}select(id='select'){this.selected=id;this.root.querySelectorAll('button').forEach(x=>x.classList.toggle('active',x.dataset.tool===id));this.onPick(id)}clear(){this.select('select')}}
...
class Editor{
 ...
 bind(){this.svg.addEventListener('pointerdown',e=>this.down(e));this.svg.addEventListener('pointermove',e=>this.move(e));this.svg.addEventListener('pointerup',e=>this.up(e));this.svg.addEventListener('contextmenu',e=>{e.preventDefault();this.cancelAction()});this.svg.addEventListener('wheel',e=>this.wheel(e),{passive:false});window.addEventListener('keydown',e=>this.key(e,true));window.addEventListener('keyup',e=>this.key(e,false))}
 ...
 cancelAction(){const hadAction=!!(this.pendingWire||this.drag||this.tool);this.pendingWire=null;this.preview=null;this.marquee=null;this.drag=null;this.tool=null;this.onCancel?.();this.render();this.onStatus(null,hadAction?'Action annulée':'Mode sélection')}
