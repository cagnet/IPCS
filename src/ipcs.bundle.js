...
class Editor{
 ...
 move(e){const p=this.point(e);this.onStatus(this.snap(p));if(this.drag?.kind==='pan'){this.pan={x:this.drag.origin.x+e.clientX-this.drag.start.x,y:this.drag.origin.y+e.clientY-this.drag.start.y};this.applyTransform()}if(this.drag?.kind==='component'){const c=this.schematic.getComponent(this.drag.id),q=this.snap({x:p.x-this.drag.offset.x,y:p.y-this.drag.offset.y});c.position=q;this.render()}if(this.drag?.kind==='marquee'){this.drag.current=p;this.marquee=this.normalizeRect(this.drag.start,p);this.render();this.onStatus(p,'Relâchez pour sélectionner les éléments entièrement contenus')}if(this.pendingWire){const q=this.constrain(this.pendingWire.points.at(-1),this.snap(p),e.shiftKey);this.preview=[...this.pendingWire.points,q];this.render()}}
