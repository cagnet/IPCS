...
class Editor{
 constructor(options){Object.assign(this,options);this.mode='EDIT';this.tool=null;this.styleId='RED';this.selection=null;this.selections=new Set();this.scale=1;this.pan={x:0,y:0};this.history=[];this.future=[];this.space=false;this.bind();this.applyTransform()}
 ...
 down(e){this.svg.focus();if(e.button===2){e.preventDefault();this.cancelAction();return}const p=this.snap(this.point(e)),terminal=e.target.dataset.terminal,id=e.target.closest('[data-id]')?.dataset.id;if(e.button===1||this.space){this.drag={kind:'pan',start:{x:e.clientX,y:e.clientY},origin:{...this.pan}};this.svg.setPointerCapture(e.pointerId);return}if(this.mode==='SIMULATION'){const c=this.schematic.getComponent(id);if(c?.type==='contact'&&c.controlType==='manual'){c.manualActivated=!c.manualActivated;this.onChange('simulation')}return}if(this.tool==='wire'){this.wireClick(terminal,p,e.shiftKey);return}if(this.tool&&!id){this.snapshot();this.add(this.tool,p);return}if(id){this.select(id);const c=this.schematic.getComponent(id);if(c){this.snapshot();this.drag={kind:'component',id,offset:{x:p.x-c.position.x,y:p.y-c.position.y}};this.svg.setPointerCapture(e.pointerId)}}else{this.drag={kind:'marquee',start:p,current:p};this.marquee=this.normalizeRect(p,p);this.svg.setPointerCapture(e.pointerId);this.render()}}
 move(e){const p=this.point(e);this.onStatus(this.snap(p));if(this.drag?.kind==='pan'){this.pan={x:this.drag.origin.x+e.clientX-this.drag.start.x,y:this.drag.origin.y+e.clientY-this.drag.start.y};this.applyTransform()}if(this.drag?.kind==='component'){const c=this.schematic.getComponent(this.drag.id),q=this.snap({x:p.x-this.drag.offset.x,y:p.y-this.drag.offset.y});c.position=q;this.syncConnectedWires(c);this.render()}if(this.drag?.kind==='marquee'){this.drag.current=p;this.marquee=this.normalizeRect(this.drag.start,p);this.render();this.onStatus(p,'Relâchez pour sélectionner les éléments entièrement contenus')}if(this.pendingWire){const q=this.constrain(this.pendingWire.points.at(-1),this.snap(p),e.shiftKey);this.preview=[...this.pendingWire.points,q];this.render()}}
 up(){if(this.drag?.kind==='component')this.onChange('move');if(this.drag?.kind==='marquee'){const rect=this.normalizeRect(this.drag.start,this.drag.current);this.selectMany(rect.width<3&&rect.height<3?[]:this.itemsInRect(rect));this.marquee=null}this.drag=null;this.render()}
 ...
 selectMany(ids){this.selections=new Set(ids);this.selection=ids.length===1?ids[0]:null;this.onSelect(this.selection,ids);this.render()}
 select(id){this.selections=new Set(id?[id]:[]);this.selection=id;this.onSelect(id,id?[id]:[]);this.render()}
 ...
 render(simulation=this.simulation){this.simulation=simulation;this.renderer.render(this.schematic,{selection:this.selection,selections:this.selections,related:this.related(),simulation,wirePreview:this.preview,marquee:this.marquee})}
 ...
 key(e,down){if(e.code==='Space'){this.space=down;if(down)e.preventDefault()}if(!down||['INPUT','SELECT'].includes(document.activeElement?.tagName))return;if(e.key==='Delete'&&this.selections.size)this.remove();if(e.key.toLowerCase()==='r'&&this.selection)this.rotate();if(e.ctrlKey&&e.key.toLowerCase()==='z'){e.preventDefault();this.undo()}if(e.ctrlKey&&(e.key.toLowerCase()==='y'||e.shiftKey&&e.key.toLowerCase()==='z')){e.preventDefault();this.redo()}if(e.ctrlKey&&e.key.toLowerCase()==='d'){e.preventDefault();this.duplicate()}}
